          ...
          <form class="layui-form">
            <div class="layui-inline" style="margin-bottom: 10px;">
              ...
              <div class="layui-input-inline">
                <input type="text" name="chongwumingcheng" id="chongwumingcheng" placeholder="宠物名称" autocomplete="off" class="layui-input">
              </div>
            </div>   
            <div class="layui-inline" style="margin-bottom: 10px;">
              <label class="layui-form-label">宠物分类</label>
              <div class="layui-input-inline">
                <input type="text" name="chongwufenlei" id="chongwufenlei" placeholder="宠物分类" autocomplete="off" class="layui-input">
              </div>
            </div>   
            <div class="layui-inline" style="margin-bottom: 10px;">
              <label class="layui-form-label">状态</label>
              <div class="layui-input-inline">
                <input type="text" name="zhuangtai" id="zhuangtai" placeholder="状态" autocomplete="off" class="layui-input">
              </div>
            </div>   
            <div class="layui-inline" style="margin-left: 30px;margin-bottom: 10px;">
              ...
              <button v-if="isAuth('chongwuxinxi','新增')" @click="jump('../chongwuxinxi/add.jsp')" type="button" class="layui-btn btn-theme">
        ...
        <div class="pro-lists-con">
          <ul class="after">
            <li v-for="(item,index) in dataList" v-bind:key="index">
              <a target="_self" :href="'../chongwuxinxi/detail.jsp?id='+item.id">
                <div class="posi-img">
                  <img :src="item.tupian?item.tupian.split(',')[0]:''" width="385px" height="220px" />
                </div>
                <div class="remark">
                  <h5>{{item.chongwumingcheng}}</h5>
  ...
  <script>
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery'], function() {
      ...
      function pageList() {
        ...
        if (jquery('#chongwumingcheng').val()) {
          param['chongwumingcheng'] = jquery('#chongwumingcheng').val() ? '%' + jquery('#chongwumingcheng').val() + '%' : '';
        }  
        if (jquery('#chongwufenlei').val()) {
          param['chongwufenlei'] = jquery('#chongwufenlei').val() ? '%' + jquery('#chongwufenlei').val() + '%' : '';
        }  
        if (jquery('#zhuangtai').val()) {
          param['zhuangtai'] = jquery('#zhuangtai').val() ? '%' + jquery('#zhuangtai').val() + '%' : '';
        }  
        // 获取列表数据
